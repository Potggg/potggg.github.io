...
const WORKER_BASE_URL = "https://github-proxy.dorangs30.workers.dev";
...
function setTries(t) {
  ...
  const epsilon = (currTries > 1) ? 1e-9 : 0;
  const correctedT = currTries + epsilon;

  const roundedTries = Math.ceil(correctedT);

  let frac;
  if (currTries >= 200) frac = 1;
  else if (currTries <= 0) frac = 0;
  ...
  document.getElementById('kpiTries').textContent =
    (Number.isFinite(t) ? roundedTries + '회' : '—');
}
  ...
  try {
    ...
    if(!(Number.isFinite(k) && u && [u.hp,u.atk,u.def,u.agi].every(Number.isFinite))){
      ...
      if(!fit){ 
        ...
        btnCalc.textContent = "계산"; btnCalc.disabled = false;
        return; 
      }
      k=fit.k; u=fit.u;
    }
